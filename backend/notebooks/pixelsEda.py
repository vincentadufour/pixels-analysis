...
def interactive_seasonal_plot():
    fig = px.line(
        data, x="month", y="score", color="year", title="Seasonal Plot of Score Across Years"
    )
    fig.show()
...
def interactive_rolling_statistics_plot():
    ...
    # rolling Mean
    fig.add_trace(
        go.Scatter(x=data.index, y=rolling_mean, mode="lines", name="30-Day Rolling Mean")
    )
    # rolling Standard Deviation
    fig.add_trace(
        go.Scatter(x=data.index, y=rolling_std, mode="lines", name="30-Day Rolling Std Dev")
    )

    fig.update_layout(
        title="Rolling Statistics Plot: Score Over Time", xaxis_title="Date", yaxis_title="Score"
    )
    fig.show()
...
def get_sentiment(text):
    return TextBlob(
        text
    ).sentiment.polarity  # returns a score between -1 and 1 as a "sentiment polarity"
    ...
    if print_note:
        ...
        for i, row in data.iterrows():
            ...
            if row["contains_term"]:
                ...
                print(
                    f"{headline1} Date: {row.name} {headline3} Score: {row['score']} {headline2}{wrapped_note}"
                )
